...
/** Número WhatsApp em formato E.164 (55 + DDD + número). */
function site_whatsapp_phone_e164(): string
{
    $cfg = function_exists('app_config') ? app_config() : [];
    $digits = preg_replace('/\D/', '', (string) ($cfg['whatsapp_number'] ?? '5511963702169')) ?? '';

    return $digits !== '' ? $digits : '5511963702169';
}

function site_status_render_whatsapp_button(): void
{
    $url = 'https://wa.me/' . site_whatsapp_phone_e164();
    $urlSafe = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
    ?>
<style>
.pp-whatsapp{position:fixed;right:20px;bottom:20px;z-index:9999;width:56px;height:56px;border-radius:50%;background:#25D366;color:#fff;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 18px rgba(0,0,0,.35);text-decoration:none;transition:transform .2s,box-shadow .2s}
.pp-whatsapp:hover{transform:scale(1.08);box-shadow:0 6px 24px rgba(37,211,102,.55);color:#fff}
.pp-whatsapp svg{display:block}
@media(max-width:480px){.pp-whatsapp{right:16px;bottom:16px;width:52px;height:52px}}
</style>
<a href="<?= $urlSafe ?>" class="pp-whatsapp" target="_blank" rel="noopener noreferrer" aria-label="Falar no WhatsApp" title="WhatsApp">
